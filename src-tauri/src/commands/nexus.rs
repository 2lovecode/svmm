...
pub fn nexus_set_key(key: String) -> AppResult<()> {
    log_result(secure_key::set_nexus_api_key(&key))
}

#[tauri::command]
pub fn nexus_clear_key() -> AppResult<()> {
    log_result(secure_key::clear_nexus_api_key())
}
...
pub fn nexus_validate() -> AppResult<NexusUser> {
    log_result(nexus::validate_nexus_api_key())
}
...
pub fn nexus_update_mod(folder_path: String) -> AppResult<scan::ModEntry> {
    log_result((|| {
        ...
        let entry = mods
            ...
            .ok_or_else(|| {
                AppError::new("mod_not_found", "未找到指定模组").with_detail(folder_path.clone())
            })?;
